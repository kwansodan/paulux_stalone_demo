...
const INSTAGRAM_LINK = "instagram.com/pauluxbooking"
    ...
    if (booking.clientPhone) {
      smsResult = await step.run("send-confirmation-sms", async () => {
        const message = isWalkIn
          ? `Welcome to Paulux! 🌟 We're so glad you're here, ${booking.clientName}. Your service(s) today include ${serviceNamesJoined}.${productNamesJoined ? ` Products: ${productNamesJoined}.` : ""} Sit back and relax — you're in great hands! Note: No-shows forfeit payment. Cancellations eligible for refund within policy. Full details: ${summaryUrl}. Instagram: ${INSTAGRAM_LINK}`
