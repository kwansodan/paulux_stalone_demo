# Paulux — Application Architecture

This document describes the technical architecture of the Paulux Booking booking platform. It covers the directory structure, data model, API design, authentication, payment system, inventory, gift cards, background jobs, deployment, and the key decisions behind each.
...
Paulux is a **full-stack Next.js 16 application** that combines a customer-facing salon booking flow with an admin management dashboard. It is a monorepo: one Next.js app serves both the public-facing website and the internal admin panel.
...
The most complex subsystem. Paulux operates with **two Paystack gateways** (primary and secondary) with automatic routing, failover, and idempotent webhook handling.
...
Inngest provides durable, retryable background functions. The server endpoint is at `/api/inngest`. The Inngest client id is `"pauluxbooking.com"` (`src/lib/inngest.ts`).
...
`src/lib/inngest.ts` creates the `Inngest` client with the app ID `"pauluxbooking.com"`. The signing key (`INNGEST_SIGNING_KEY`) and event key (`INNGEST_EVENT_KEY`) authenticate communication with the Inngest cloud. `INNGEST_DEV` toggles local dev mode.

---

## 14. File Storage — MinIO

`src/lib/minio.ts` creates a MinIO client pointed at the internal Docker service (`MINIO_ENDPOINT`, `MINIO_PORT`). Bucket: `paulux-uploads`. Policy: public-read.
...
Admin uploads image →
  POST /api/upload/presign
    ...
    → Rewrites internal host to public URL (pauluxbooking.com/files)
...
`ServiceRepository.getAllServices()` silently rewrites image URLs containing the old domain `pauluxbooking.com` to the current domain `pauluxbooking.com`, maintaining backward compatibility after a domain migration without a database update.
...
All services share `paulux_network` (bridge). The app container never exposes port 3000 publicly — only Nginx does, on 80/443.
