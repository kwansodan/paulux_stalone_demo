...
export async function POST(req: NextRequest) {
    try {
        ...
        // Verify signature against SECONDARY_PAYSTACK — must hash the raw bytes Paystack signed
        const isValid = verifyPaystackSignature(signature, rawBody, PaymentProvider.SECONDARY_PAYSTACK);

        if (!isValid) {
            console.error('Invalid SECONDARY Paystack webhook signature');
        ...
        console.log(`Received SECONDARY Paystack event: ${event}`);
        ...
        if (invoice) {
            ...
            await auditLogService.logAction({
                ...
                metadata: { provider: PaymentProvider.SECONDARY_PAYSTACK, event, body }
            });
        }
        ...
        if (event === 'charge.success') {
            ...
            if (status === 'success') {
                try {
                    ...
                    console.log(`Payment successfully processed via SECONDARY webhook for reference: ${reference}`);
                } catch (serviceError: any) {
                    console.error('Error in payment service (SECONDARY):', serviceError);
                }
            }
        }
        ...
        if (event === 'charge.failed') {
            ...
            console.log(`Payment failed for reference: ${reference} (SECONDARY)`);
            ...
            if (payment) {
                ...
                console.log(`Updated payment ${payment.id} and booking to FAILED (SECONDARY)`);
            }
        }
        ...
        if (event === 'refund.processed') {
            ...
            console.log(`Refund processed for transaction: ${transaction_reference} (SECONDARY)`);
            ...
            if (payment) {
                ...
                console.log(`Updated payment ${payment.id} to REFUNDED (SECONDARY)`);
            }
        }
        ...
        console.error('SECONDARY Webhook processing error:', error);
        return NextResponse.json(
