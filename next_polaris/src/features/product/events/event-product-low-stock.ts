    ...
    if (adminsWithPhone.length > 0) {
      await step.run("send-sms-alerts", async () => {
        const message = isOutOfStock
          ? `POLARIS ALERT: "${productName}" is now OUT OF STOCK. Please restock immediately.`
          : `POLARIS ALERT: "${productName}" is running low — ${currentStock} unit${currentStock === 1 ? "" : "s"} left (threshold: ${threshold}). Please restock soon.`
