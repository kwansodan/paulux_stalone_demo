    ...
    if (adminsWithPhone.length > 0) {
      await step.run("send-sms-alerts", async () => {
        const message = isOutOfStock
          ? `POLARIS ALERT: material "${materialName}" is now OUT OF STOCK. Please restock immediately.`
          : `POLARIS ALERT: material "${materialName}" is running low — ${currentStock} ${unit} left (threshold: ${threshold}). Please restock soon.`
