...
1. SSH into the VPS:
   ...
   ```
2. Run this command to find the internal IP address of the `paulux_db` container:
   ```bash
   docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' paulux_db
   ```
...
3. In the **General** tab, name it something like `Paulux Production DB`.
